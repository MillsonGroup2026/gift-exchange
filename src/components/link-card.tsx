...
export function LinkCard({
  ...
  const hasImage = !!(meta?.image && imgOk);
  ...
  return (
    <a
      ...
      className="block overflow-hidden rounded-xl border border-border bg-background transition-colors hover:border-brand/40"
    >
      {hasImage && (
      ...
      <div className="flex items-center gap-3 p-3">
        {!hasImage && (
          <span className="grid h-10 w-10 flex-none place-items-center rounded-lg bg-muted">
            {meta?.favicon && favOk ? (
              // eslint-disable-next-line @next/next/no-img-element
